...
{
    XHomeDoc* pDoc = DYNAMIC_DOWNCAST(XHomeDoc, pContext->m_pCurrentDoc);
...
void CMainFrame::ActivateCategory (CBCGPRibbonCategory* pCategory)
{
	...
	else if (pCategory == m_pRibbonCategory[e_ModeOrder])
	{
		nShortcut = 8;
		mode = e_ModeOrder;
	}
...
BOOL CMainFrame::CreateRibbonBar ()
{
	...
	m_pRibbonCategory[e_ModeOrder] = AddTab_Order ();
...
CBCGPRibbonCategory* CMainFrame::AddTab_Order()
{
	CBCGPRibbonCategory* pCategory = m_wndRibbonBar.AddCategory (
		c_ViewNames[e_ModeOrder], IDB_MAIL_SMALL, IDB_MAIL_LARGE);
	pCategory->SetKeys (_T("o"));
...
void CMainFrame::OnBeforeChangeVisualTheme(CBCGPWinApp::BCGP_VISUAL_THEME theme)
{
	...
	m_wndRibbonBar.SetBackstageMode(bIsOffice2010Look || bIsOffice2013Look);

	if (bIsOffice2013Look)
	{
...
void CMainFrame::OnAfterChangeVisualTheme(CBCGPWinApp::BCGP_VISUAL_THEME theme)
{
	...
	UINT nWaterMarkResID = (theme >= CBCGPWinApp::BCGP_VISUAL_THEME_OFFICE_2013_WHITE && theme <= CBCGPWinApp::BCGP_VISUAL_THEME_OFFICE_2013_DARK_GRAY) ? 0 : IDB_BS_WATERMARK;
...
{
	XHomeDoc* pDoc = (XHomeDoc*) GetActiveDocument ();
	if (pDoc != NULL)
	{
		ASSERT_VALID (pDoc);
...
void CMainFrame::ShowOptions (int nPage)
{
	...
	propSheet.SetLook (CBCGPPropertySheet::PropSheetLook_List, 150);
